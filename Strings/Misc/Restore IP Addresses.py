...
class Solution:
    def restoreIpAddresses(self, s: str) -> List[str]:
        result = []
        self.doRestore(result, "", s, 0)
        return result
    
    def doRestore(self, result, path, s, k):
        if not s and k == 4:
        ...
        for i in range(1, 4):
            if i > len(s):
                break
            part = s[:i]
            if len(part) > 3 or (part[0] == '0' and len(part) > 1) or int(part) > 255:
                continue
            self.doRestore(result, path + "." + part, s[i:], k + 1)
