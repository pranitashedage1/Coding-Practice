...
sequence of bits so that it can be stored in a file or memory buffer, or transmitted across a 
...
each node has no more than N children. There is no restriction on how your serialization/deserialization algorithm should work. You just need to ensure that an N-ary tree can be serialized to a string and 
...
class Codec:
    ...
    def deserialize(self, data):
        ...
        def BuildTree(queue):
            ...
            root = Node(node, [])
            for _ in range(size):
                root.children.append(BuildTree(queue))
            return root

        queue = deque(data.split(self.split))
