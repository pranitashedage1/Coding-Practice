...
the sum of the node values in the path equals targetSum. Each path should be returned as a list of 
...
class Solution:
    ...
    def path(self, lst: List[int], root: TreeNode, sum: int) -> None:
        if not root:
            return
        lst.append(root.val)
        if not root.left and not root.right and sum - root.val == 0:
            self.res.append(lst[:])
        self.path(lst, root.left, sum - root.val)
        self.path(lst, root.right, sum - root.val)
        lst.pop()
