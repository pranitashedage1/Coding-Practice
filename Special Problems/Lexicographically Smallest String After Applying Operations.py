...
class Solution:
    def findLexSmallestString(self, s: str, a: int, b: int) -> str:
        # create BFS queue for all possibilities
        queue = deque([s])

        # Set to track visited strings
        visited = set([s])

        # set smallest string s
        smallest = s
        ...
        while queue:
            ...
            # step 1 adding a in all odd digits
            current_list = list(current)
            for i in range(1, len(s), 2):
                current_list[i] = str((int(current_list[i]) + a) % 10)
            
            add_a_result = ''.join(current_list)
            if add_a_result not in visited:
                queue.append(add_a_result)
                visited.add(add_a_result)

            # step 2 moving string with b poisitons
            rotate_b_result = current[-b:] + current[:-b]

            if rotate_b_result not in visited:
                queue.append(rotate_b_result)
                visited.add(rotate_b_result)
