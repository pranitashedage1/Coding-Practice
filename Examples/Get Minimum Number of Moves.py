...
The equipment you want has blocks of many different weights that you can combine to lift.
The listing on Amazon gives you an array, blocks , that consists of n different weighted blocks, in kilograms. 
There are no two blocks with the same weight. 

The element blocks[i] denotes the weight of the i th block from the top of the stack. 
You consider weight lifting equipment to be good if the block at the top is the lightest, 
and the block at the bottom is the heaviest.

More formally, the equipment with array blocks will be called good weight lifting equipment if it 
satisfies the following conditions assuming the index of the array starts from 1:
...
def getMinNumMoves(blocks):
    ...
    # Calculate the swaps needed to move the smallest block to the first position
    min_swaps = min_index
    
    # Calculate the swaps needed to move the largest block to the last position
    max_swaps = n - 1 - max_index
    
    # If the smallest block is to the right of the largest block initially, adjust the count
    if min_index > max_index:
        return min_swaps + max_swaps - 1
    else:
...
blocks3 = [3, 2, 1]
print(getMinNumMoves(blocks3))  # Output: 3
